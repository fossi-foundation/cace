...
paramkey_paramdisplay = {
    i: j.get("display", i) for i, j in parameter_manager.datasheet["parameters"].items()
}
paramdisplay_paramkey = {
    j.get("display", i): i for i, j in parameter_manager.datasheet["parameters"].items()
}
...
def runsim():
    ...
    for pname in params:
        parameter_manager.queue_parameter(
            ...
            step_cb=lambda param: (task_queue.put({"task": "step", "param": param})),
            cancel_cb=lambda param: (
                task_queue.put({"task": "cancel", "param": param})
            ),
            end_cb=lambda param: (task_queue.put({"task": "end", "param": param}),),
        )
    ...
    parameter_manager.set_runtime_options("netlist_source", rd["netlist_source"])
...
def simresults():
    ...
    lengths = {
        param: len(
            list(parameter_manager.datasheet["parameters"][param]["spec"].keys())
        )
    ...
    for pname in figures.keys():
        ...
        for figure in figures[pname]:
            ...
            divs.append(
                mpld3.fig_to_html(fig, include_libraries=False, template_type="simple")
            )
