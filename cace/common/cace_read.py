        ...
        for eparam in curdict["electrical_parameters"]:
            ...
            if "spec" in datasheet["parameters"][eparam["name"]]:
                for limit in ["minimum", "typical", "maximum"]:
                    if limit in datasheet["parameters"][eparam["name"]]["spec"]:
                        spec = datasheet["parameters"][eparam["name"]]["spec"].pop(
                            limit
                        )
                        datasheet["parameters"][eparam["name"]]["spec"][limit] = {}
            ...
            if "simulate" in datasheet["parameters"][eparam["name"]]:
                if "format" in datasheet["parameters"][eparam["name"]]["simulate"]:
                    format = datasheet["parameters"][eparam["name"]]["simulate"].pop(
                        "format"
                    )
                    datasheet["parameters"][eparam["name"]]["simulate"]["format"] = (
                        format[0]
                    )
                    datasheet["parameters"][eparam["name"]]["simulate"]["suffix"] = (
                        format[1]
                    )
                    datasheet["parameters"][eparam["name"]]["simulate"]["variables"] = (
                        format[2:]
                    )

                toolname = datasheet["parameters"][eparam["name"]]["simulate"].pop(
                    "tool"
                )

                if "template" in datasheet["parameters"][eparam["name"]]["simulate"]:
                ...
                datasheet["parameters"][eparam["name"]]["tool"] = {
                    toolname: datasheet["parameters"][eparam["name"]].pop("simulate")
                }
    ...
    if "physical_parameters" in curdict:
        for pparam in curdict["physical_parameters"]:
            ...
            if "spec" in datasheet["parameters"][pparam["name"]]:
                for limit in ["minimum", "typical", "maximum"]:
                    if limit in datasheet["parameters"][pparam["name"]]["spec"]:
                        spec = datasheet["parameters"][pparam["name"]]["spec"].pop(
                            limit
                        )
                        datasheet["parameters"][pparam["name"]]["spec"][limit] = {}
            ...
            if "evaluate" in datasheet["parameters"][pparam["name"]]:
                toolname = datasheet["parameters"][pparam["name"]]["evaluate"].pop(
                    "tool"
                )
                datasheet["parameters"][pparam["name"]]["tool"] = {
                    toolname: datasheet["parameters"][pparam["name"]].pop("evaluate")
                }
...
def validate_datasheet(datasheet):
    ...
    if not "cace_format" in datasheet:
        warn(f"No cace_format given, trying to read as {CACE_DATASHEET_VERSION}.")
