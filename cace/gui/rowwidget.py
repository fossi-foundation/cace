...
class RowWidget:
    ...
    def set_functions(self, start, stop, edit, copy, delete, failreport, textreport):
    ...
    def min_limit_text(self):
        ...
        if "minimum" in specdict:
            ...
            if pmin != "any":
                if "unit" in self.param and not binrex.match(self.param["unit"]):
    ...
    def create_widgets(self, dframe, n):
        ...
        # LVS
            ...
            self.status_widget = ttk.Button(
                ...
                text=status_value,
                style=button_style,
                command=lambda lvs_file=lvs_file: self.fnc_textreport(lvs_file),
            )
    ...
    def update_widgets(self):
        ...
        if self.is_plot:
            ...
            min_value, min_status_style = self.min_value_text()
            self.min_value_widget.configure(text=min_value, style=min_status_style)

            # Typical widgets
            self.typ_limit_widget.configure(text=self.typ_limit_text())
            typ_value, typ_status_style = self.typ_value_text()
            self.typ_value_widget.configure(text=typ_value, style=typ_status_style)

            # Maximum widgets
            self.max_limit_widget.configure(text=self.max_limit_text())
            max_value, max_status_style = self.max_value_text()
            self.max_value_widget.configure(text=max_value, style=max_status_style)
